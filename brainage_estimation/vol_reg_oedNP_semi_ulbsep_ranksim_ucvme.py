...
def run(
    ...
    if device.type == "cuda":
        model = models.generate_model_ucvme(model_depth=18, n_classes=1, )
        print("model.fc.bias.data[0]", model.fc.bias.data[0])
        model = torch.nn.DataParallel(model)

        
        model_1 = models.generate_model_ucvme(model_depth=18, n_classes=1)
...
def run_epoch(model, 
    ...
    for train_iter in range(total_itr_num):
        ...
        all_output_0 = model[0](X0)        
        all_output_1 = model[1](X1)

        mean_raw, var_raw = all_output_0
        ...
        yhat_0.append(all_output_0[0].view(-1).to("cpu").detach().numpy() * y_std + y_mean)
        ...
        all_output_unlb_0_pred_0, var_unlb_0_pred_0 = model[0](X_ulb_0)
        all_output_unlb_1_pred_0, var_unlb_1_pred_0 = model[1](X_ulb_1)
        ...
        with torch.no_grad():
            for samp_ssl_itr in range(samp_ssl):
                mean1_raw_0, var1_raw_0 = model[0](X_ulb_0)
                ...
                mean1_raw_1, var1_raw_1 = model[1](X_ulb_1)
        ...
        cps_loss = (loss_reg_cps0 + loss_reg_cps1) + (var_loss_ulb_0 + var_loss_ulb_1)
        
        loss_ulb_0 = loss_reg * 0
        loss_ulb_1 = loss_reg * 0

        loss = loss_reg + w_cps * cps_loss + ((var_1 - var) ** 2).mean()
