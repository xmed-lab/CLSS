...
### copied from vol_reg_unc_unali on 2022/02/08, code for zdbg_out_220202nxp_vlrgnmuncaliumse_w1_epc30_m2_eght_lr3_drp02_s0_mps_bnd6_ulbcmb_smi. We get rid of the /2s in the losses 
### 
...
# from models.ulb_rank_ssl import ulb_rank, ulb_rank_prdlb
from models.ulb_rank_ssl_mixed import ulb_rank, ulb_rank_prdlb
from models.ulb_rank_ssl_mixed import ulb_rank_cps, ulb_rank_prdlb_cps
...
@click.option("--data_dir", type=click.Path(exists=True, file_okay=False), default="/data1/cewang/Code/GCLSS/utk/DATA_DIR")
...
@click.option("--y_mean", type=float, default=49)
@click.option("--y_std", type=float, default=17)
...
def run(
    # data_dir="/home/eecewang/Code/DR-project/CLSS-main/brainage_estimation/BRAIN",
    data_dir="DATA_DIR",
    output=None,
    task="age",
    ...
    if device.type == "cuda":
        ...
        # model = models.generate_model(model_depth=18, n_classes=1)
    ...
    mean, std = utils.get_mean_and_std(datasets.UTKdta(root=data_dir, split="train")) # We can use whole dataset to get mean std, because not label dependent
    ...
    if reduced_set:
        ...
        dataset_trainsub['lb'] = datasets.UTKdta(root=data_dir, split="train", **kwargs, pad=pad_param, ssl_postfix="_ssl_{}_{}".format(rd_label, rd_unlabel), ssl_type = 1, ssl_mult = ssl_mult_choice) # exclude_split = excsplt
        dataset_trainsub['unlb_0'] = datasets.UTKdta(root=data_dir, split="train", **kwargs, pad=pad_param, ssl_postfix="_ssl_{}_{}".format(rd_label, rd_unlabel), ssl_type = 2) # exclude_split = excsplt
    else:
            ...
            for phase in ['train', 'val']:
                ...
                if phase == "train":
                    ...
                    dataloader_unlb_0 = torch.utils.data.DataLoader(
                        ds['unlb_0'], batch_size=batch_size_two, num_workers=num_workers, shuffle=True, pin_memory=(device.type == "cuda"), drop_last=(phase == "train"), worker_init_fn=worker_init_fn)
                    ...
                    loss_tr, loss_reg_0,  ctr_ls, ctr_ulbls, yhat_0, y = run_epoch(model = model, 
                    ...
                    for val_samp_itr in range(val_samp):
                        ...
                        loss_valit, loss_ctr, yhat, y = run_epoch_val_avg(model = model, 
            ...
            for split in split_list: ### for CAMUS
                ...
                if not full_test:
                    ...
                    for seed_itr in range(1):
                        ...
                        ds = datasets.UTKdta(root=data_dir, split=split, **kwargs, ssl_postfix="_ssl_{}_{}".format(rd_label, rd_unlabel))
                        dataloader = torch.utils.data.DataLoader(
                            ds,
                            batch_size=batch_size, num_workers=num_workers, shuffle=False, pin_memory=(device.type == "cuda"), worker_init_fn=worker_init_fn)
                        total_loss, _, yhat, y = run_epoch_val_avg(model = model, 
                                                                    dataloader = dataloader, 
                                                                    train = False, 
                                                                    optim = None, 
                                                                    device = device, 
...
def run_epoch(model, 
    ...
    # print(len(dataloader_lb), len(dataloader_unlb_0))
    # exit()
    ...
    output_l_memory = []
    output_l_feat_memory = []
    output_ul_memory = []
    output_ul_feat_memory = []
    for train_iter in range(total_itr_num):

        # (X0, X1, outcome ) = next(dataloader_lb_itr)
        (X0, outcome ) = next(dataloader_lb_itr)
        ...
        # (X_ulb_0, X_ulb_1, outcome_ulb) = next(dataloader_unlb_0_itr)
        ...
        for num in range(2):
            ...
            if num%2 == 0:
                all_output, all_output_feat = model(X0)
                all_output_memory = all_output.clone().detach()
                all_output_feat_memory = all_output_feat.clone().detach()
                output_l_memory.append(all_output_memory)
                output_l_feat_memory.append(all_output_feat_memory)
                loss_ctr = criterion_cntrst(all_output_feat,  (outcome - y_mean) / y_std)


                loss_reg_0 = torch.nn.functional.mse_loss(all_output.view(-1), (outcome - y_mean) / y_std )
                # yhat_0.append(all_output.view(-1).to("cpu").detach().numpy() * y_std + y_mean)        

                loss_reg = loss_reg_0

                
                all_output_unlb_0_pred_0, all_output_unlb_0_feat_0 = model(X_ulb_0)
                all_output_unlb_0_pred_0_memory = all_output_unlb_0_pred_0.clone().detach()
                all_output_unlb_0_feat_0_memory = all_output_unlb_0_feat_0.clone().detach()
                output_ul_memory.append(all_output_unlb_0_pred_0_memory)
                output_ul_feat_memory.append(all_output_unlb_0_feat_0_memory)

                if w_ctrulb_0 > 0:
                    # assert False, "Not Done yet"
                    loss_ulb_0, ft_rank = ulb_rank(all_output_unlb_0_feat_0, all_output_feat, outcome, lambda_val)
                    # print(ft_rank)
                    if w_ctrulb_1 > 0:
                ...
                loss = loss_reg + w_ctr * loss_ctr + w_ctrulb_0 * loss_ulb_0 + w_ctrulb_1 * loss_ulb_1
                ...
                all_output, all_output_feat = model(X0)
                output_l_memory.append(all_output)
                output_l_feat_memory.append(all_output_feat)
                loss_ctr = criterion_cntrst(all_output_feat,  (outcome - y_mean) / y_std)
                ...
                all_output_unlb_0_pred_0, all_output_unlb_0_feat_0 = model(X_ulb_0)
                output_ul_memory.append(all_output_unlb_0_pred_0)
                output_ul_feat_memory.append(all_output_unlb_0_feat_0)

                if w_ctrulb_0 > 0:
                    # assert False, "Not Done yet"
                    # loss_ulb_0, ft_rank = ulb_rank(all_output_unlb_0_feat_0, all_output_feat, outcome, lambda_val)
                    loss_ulb_0, ft_rank,samples,r_value = ulb_rank_cps(output_ul_feat_memory[0], output_ul_feat_memory[1], output_l_feat_memory[0], output_l_feat_memory[1],outcome,lambda_val, batch_size_two)
                    # print(ft_rank)
                    if w_ctrulb_1 > 0:
                        # loss_ulb_1 = ulb_rank_prdlb(all_output_unlb_0_pred_0, lambda_val, pred_inp=ft_rank)
        ...
        if train_iter % 10 == 0:
            # break
            print("phase {} itr {}/{}: ls {:.2f}({:.2f}) rg0 {:.4f} ({:.2f}) ctr {:.6f} ({:.6f}) ctrulb {:.7f} ({:.2f})  ctrulbPSL {:.7f} ({:.2f})    ".format(train,
    ...
    return total / n, total_reg / n, total_ctr / n, total_ctrulb_0 / n, yhat_0, y
