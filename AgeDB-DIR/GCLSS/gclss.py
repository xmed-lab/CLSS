...
# from models.ulb_rank_ssl import ulb_rank, ulb_rank_prdlb, mix_rank_func, mix_rank_prdlb_func
...
@click.option("--data_dir", type=click.Path(exists=True, file_okay=False), default="/data1/cewang/Code/pami/GCLSS/age_estimation/AgeDB_DIR")
...
@click.option("--num_epochs", type=int, default=2)   #### 30 is enough for full set, best usually not over 30
...
def run(
    ...
    data_dir="/home/eecewang/Code/DR-project/CLSS-main/age_estimation/AgeDB_DIR",
    output=None,
    task="age",
        ...
        for epoch in range(epoch_resume, num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                if phase == "train":
                    ...
                    loss_tr, loss_reg_0,  ctr_ls, ctr_ulbls, yhat_0, y, test_values_curr_epoch = run_epoch(model = model, 
                    ...
                    test_values['Epoch{}'.format(epoch)] = test_values_curr_epoch
            ...
            scheduler.step()
            np.save('./test_values32', test_values)
...
def run_epoch(model, 
    ...
    ## dataloader of the labeled data
    dataloader_lb_itr = iter(dataloader_lb)
    ## dataloader of the unlabeled data
    ...
    test_values_curr_epoch = []
    for train_iter in range(total_itr_num):

        # video1, video2, target, target_cls
        ...
        # computing corresponding losses for unlabeled data ranking
        ...
        for num in range(2):
            ...
            if num%2 == 0:
                ...
                if w_ctrulb_0 > 0:
                    ...
                    ## UR
                    # loss_ulb_0, ft_rank = ulb_rank(all_output_unlb_0_feat_0, lambda_val)
                    # loss_ulb_0, ft_rank, r = ulb_rank(all_output_unlb_0_feat_0, all_output_feat, outcome, lambda_val)
                    ...
                    if w_ctrulb_1 > 0:
                        ...
                        # loss_ulb_1 = ulb_rank_prdlb(all_output_unlb_0_pred_0, r, lambda_val, pred_inp=ft_rank)
                        loss_ulb_1 = ulb_rank_prdlb(all_output_unlb_0_pred_0, lambda_val, pred_inp=ft_rank)
                    else:
                ...
                # computing corresponding losses for labeled data ranking
                # loss_mix_rank = mix_rank_func(all_output_feat, all_output_unlb_0_feat_0, lambda_val)
                ...
                if w_ctrulb_0 > 0:
                    # assert False, "Not Done yet"
                    # loss_ulb_0, ft_rank = ulb_rank(all_output_unlb_0_feat_0, all_output_feat, outcome, lambda_val)
                    loss_ulb_0, ft_rank,samples,r_value, test_value = ulb_rank_cps(output_ul_feat_memory[0], output_ul_feat_memory[1], output_l_feat_memory[0], output_l_feat_memory[1],outcome,lambda_val, batch_size_two)
                    # print(ft_rank)
    ...
    return total / n, total_reg / n, total_ctr / n, total_ctrulb_0 / n, yhat_0, y, test_values_curr_epoch
